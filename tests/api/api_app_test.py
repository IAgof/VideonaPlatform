...
from tests.factories import UserFactory, PromoCodeFactory
...
class TestAPIAppSetUp(object):
    ...
    @mock.patch('flask_jwt._jwt_required', side_effect=mock_jwt_required)
    ...
    # ------------- promo_codes -----------
    def test_api_promo_codes_blueprint_definition(self, api_app):
        assert_that(promo_codes_blueprint, not_none())
        assert_that(promo_codes_blueprint, instance_of(Blueprint))
        assert_that(promo_codes_blueprint.name, is_("promo_codes"))
        assert_that(promo_codes_blueprint.url_prefix, is_("/v1/promo_codes"))
        assert_that(promo_codes_blueprint.name, is_in(api_app.blueprints))
